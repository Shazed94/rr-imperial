...
const AdminDivisions = () => {
    ...
    const storeData = (e) => {
        ...
        formdata.append("name", divisionRef.current.value);

        create_new_division(formdata).then((response) => {
    ...
    //=================================== Edit Division ===================================
    const [editDivisionVal, setEditDivisionVal] = useState();
    const [editedEventId, setEditedEventId] = useState();
    const editEvent = (divisionId, modalValue) => {
        edit_division(divisionId).then((res) => {
            setEditedEventId(divisionId);
            setEditDivisionVal(res.data.edit_division);
            setModalData(modalValue);
            setOpen(true);
        });
    };
    ...
    const updateDivision = (e) => {
        ...
        update_division(editedEventId, formdata).then((response) => {
            if (response.data.status === 200) {
                Swal.fire({
                    icon: "success",
                    text: response.data.message,
    ...
    const TABLE_HEAD = ["#", "Divisions", "Handle"];
    return (
        // <AdminMaster>
            ...
            <div className="content-wrapper">
                <div className="breadcrumb flex justify-between shadow-lg shadow-cyan-400/80">
                    <div className="breadcrumb-item flex items-center ">
                        ...
                        <div className="before">Divisions</div>
                    </div>
                </div>
                                ...
                                <div className="grid grid-cols-12 gap-4">
                                    <div className="mb-1 flex flex-col gap-2 col-span-12">
                                        ...
                                        <input
                                            id="divisionRef"
                                            ref={divisionRef}
                                            defaultValue={editDivisionVal?.name}
                                            type="text"
                                            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            placeholder="Division Name"
                                            required
